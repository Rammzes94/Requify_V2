...
2. Choose between normal or dry-run mode
3. Set up the database before running the pipeline
...
def run_pipeline_interactive() -> None:
    ...
    # Select dry run mode
    dry_run = input("\nRun in dry mode (check without saving to database)? (y/n): ").lower() == 'y'
    ...
    if input("\nConfirm these settings? (y/n): ").lower() != 'y':
        print("Pipeline run cancelled. Exiting.")
        return
    
    # Run the pipeline
    print("\n=== Starting Pipeline ===")
    try:
        success = process_document(input_file, max_step=max_step, dry_run=dry_run)
        if success:
...
def main() -> None:
    ...
    parser.add_argument("--dry-run", action="store_true", help="Run in dry mode (no database changes)")
    ...
    try:
        success = process_document(args.input, max_step=args.max_step, dry_run=args.dry_run)
        if success:
...
def test_with_hardcoded_file():
    ...
    try:
        ...
        dry_run = False
        success = process_document(test_file, max_step=max_step, dry_run=dry_run)
...
if __name__ == "__main__":
    # Run the test with hardcoded file if requested
