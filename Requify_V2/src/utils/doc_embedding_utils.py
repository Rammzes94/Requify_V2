...
def prepare_document_text(all_records: List[Dict]) -> str:
    ...
    try:
        ...
        if tokens_count > config.DOC_EMBEDDING_MAX_SEQ_LENGTH:
            # Truncate tokens and decode back to text
            logger.info(f"Document text exceeds token limit ({tokens_count} > {config.DOC_EMBEDDING_MAX_SEQ_LENGTH}). Truncating.", extra={"icon": "✂️"})
            truncated_tokens = tokens[:config.DOC_EMBEDDING_MAX_SEQ_LENGTH]
            truncated_text = tokenizer.decode(truncated_tokens)
            logger.info(f"Truncated document text to {config.DOC_EMBEDDING_MAX_SEQ_LENGTH} tokens", extra={"icon": "✅"})
            return truncated_text
        else:
            logger.info(f"Document text is within token limit ({tokens_count} <= {config.DOC_EMBEDDING_MAX_SEQ_LENGTH})", extra={"icon": "✅"})
            return full_text
...
def get_document_embedder(device: str = "cpu") -> Optional[SentenceTransformer]:
    ...
    """
    try:
        logger.info(f"Loading document embedding model: {config.DOC_EMBEDDING_MODEL_NAME} on {device}", extra={"icon": "🧠"})
        model = SentenceTransformer(config.DOC_EMBEDDING_MODEL_NAME, device=device, trust_remote_code=True)
        model.max_seq_length = config.DOC_EMBEDDING_MAX_SEQ_LENGTH  # Explicitly set max sequence length
        logger.info(f"Document embedding model loaded successfully with max_seq_length={config.DOC_EMBEDDING_MAX_SEQ_LENGTH}", extra={"icon": "✅"})
        return model
    except Exception as e:
