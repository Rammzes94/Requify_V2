...
passed=0
failed=0
total=6  # Update this if you change the number of scenarios

# Run each scenario
for i in $(seq 1 $total); do
  echo
  echo "===== Running Scenario $i ====="
  if python _02_src/e2e_test_scenarios.py --scenario $i; then
...
if [ -f "_02_src/test_results_reporter.py" ]; then
  echo "Generating HTML report..."
  python _02_src/test_results_reporter.py
