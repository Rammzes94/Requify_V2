...
# Add parent directory to path to allow importing from _02_src
...
def check_test_files():
    ...
    try:
        # Import and run test file checker
        from _02_src.check_test_files import check_test_files as checker
        return checker()
    except ImportError:
        print("❌ Could not import test file checker")
        return False
...
def print_scenarios():
    ...
    print("3: Extra End Page")
    print("4: Reordered Specification")
    print("5: Unique Original vs. Unique Reordered")
    print("6: Language Variant")
    print("7: Changed vs. Extra vs. Reordered (Combined Test)")
...
def run_scenario(scenario_id, generate_report=True):
    ...
    cmd = [sys.executable, "_02_src/e2e_test_scenarios.py", "--scenario", str(scenario_id)]
...
def run_all_scenarios(generate_report=True):
    ...
    cmd = [sys.executable, "_02_src/e2e_test_scenarios.py"]
    result = subprocess.run(cmd)
    success = result.returncode == 0
...
def run_subset(generate_report=True):
    ...
    subset = [1, 2, 5]  # Basic duplicate, changed values, unique documents
...
def generate_html_report():
    ...
    try:
        ...
        from _02_src.test_results_reporter import main as generate_report
        generate_report()
        print("✅ HTML report generated")
        return True
    except ImportError:
        print("❌ Could not import HTML report generator")
        return False
